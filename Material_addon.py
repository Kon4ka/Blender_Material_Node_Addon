            ...
            if material.use_nodes:
                ...
                if unused_nodes:
                    ...
                    for node in unused_nodes:
                        log_message(f"  Node: {node.name}, Type: {node.type} is not used. Located in material {material.name}")
            else:
    ...
    try:
        for node_group in bpy.data.node_groups:
            ...
            if unused_nodes:
                ...
                for node in unused_nodes:
                    log_message(f"  Node: {node.name}, Type: {node.type} is not used in node group {node_group.name}")
                if node_group.name in node_groups_usage:
                    log_message(f"  -- Used in materials: {', '.join(node_groups_usage[node_group.name])}")
    except Exception as e:
